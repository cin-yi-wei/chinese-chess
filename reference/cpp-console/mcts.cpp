...
unsigned short mcts::search(Treenode *node){
        ...
        qsrand(t.msec()*1000+t.second());

        int computation_budget = 10000;
        for(int i=0;i<computation_budget;i++){
//     qDebug()<<"times"<<i<<node->visits<<node->move;
        node =select(node);
    //  qDebug()<<node->move;
   ...
   // QVector::iterator Destructor;
    //QVectorIterator<Treenode> i(AllTreeVectorForDestructor);


    if(node->move!=1333)  qDebug()<<"errorrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr";

   //   qDebug()<<"root"<<node->visits;
     //qDebug()<<"children"<<node->children[0].visits;
    //Treenode *best_next_node = best_child(node);
...
unsigned short mcts::next_move(Treenode *node){
   ...
   // qDebug()<< "best_child1";
    ...
    //qDebug()<<"AllTreeVectorForDestructor"<< AllTreeVectorForDestructor.size() ;
     QVector<Treenode *>::iterator iter;
