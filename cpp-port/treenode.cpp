...
#include "float.h"
#include <cstdlib>
Treenode::Treenode(unsigned short move, std::set<unsigned short> moves,Treenode *parentnode,std::vector<Treenode*> *Destructor)
{
    this->move=move;
...
bool Treenode::is_all_expand( int move_length ) {
   // qDebug()<< this->children.length() << this->children.size() <<this->children.count() ;
    ...
    if( this->children.size() == move_length ) return true;
    else return false;
    /*
...
unsigned short Treenode::random_set_element( std::set<unsigned short>& s){
    int r = rand() % s.size();
   // qDebug()<<s.size()<<r;
      std::set<unsigned short>::iterator it = s.begin();
      for (; r != 0; r--) it++;
      //qDebug()<< *it ;
      return *it;
}

Treenode *Treenode::expand_move(Treenode *node,Position *pos,std::vector<Treenode*> *Destructor){
    unsigned short move= random_set_element(node->moves);
    this->moves.erase(move);
 ...
 if(times==all_moves_count ){node->gameover=true; return node;}
        ...
        std::vector<unsigned short> v=pos->generateMoves();
       ...
       auto t=v.begin();
        ...
        this->children.push_back(sub_node);
        Destructor->push_back(sub_node);
    /*
        if(set.size()==0){
            sub_node->leaf=true;
            qDebug()<<"game is over";
