...
void Position::setIrrev() {
    ...
    this->mvList.push_back(0);	// 存放每步走法的数组
  this->pcList.push_back(0);	// 存放每步被吃的棋子。如果没有棋子被吃，存放的是0
...
void Position::movePiece(unsigned short mv) {
  ...
  this->pcList.push_back(pc);			// 将终点位置的棋子，存入吃子列表
  if (pc > 0) {
    // 终点有棋子，则要删除该棋子
  ...
  this->mvList.push_back(mv);					// 将走法存入走法列表
}
void Position::undoMovePiece() {
  unsigned short mv = this->mvList.back(); this->mvList.pop_back();
  ...
  pc = this->pcList.back(); this->pcList.pop_back();
...
bool Position::makeMove (unsigned short mv) {
  ...
  this->keyList.push_back(this->zobristKey);		// 存储局面的zobristKey校验码
  this->changeSide();		// 切换走棋方
  this->chkList.push_back(this->checked());
  this->distance ++;			// 棋局深度+1
  return true;
}
void Position::undoMakeMove() {
        this->distance --;	// 棋局深度减1
        this->chkList.pop_back();
        this->changeSide();	// 切换走棋方
...
bool Position::isMate() {
   std::vector<unsigned short> mvs = this->generateMoves();
...
//    qsrand(t.msec()*1000);
  ...
  int length=this->generateMoves().size();
...
std::vector< unsigned short> Position::generateMoves () {
  std::vector< unsigned short> mvs;									// 用于存储所有合法的走法
...
int index = this->mvList.size();
if(index>=13){
    index = this->mvList.size()-5;
    if (this->mvList[index] == this->zobristKey) return true;

    index = this->mvList.size() - 9;
    if (this->mvList[index] == this->zobristKey) return true;

    index = this->mvList.size() - 13;
    if (this->mvList[index] == this->zobristKey) return true;
}else return  false;
index = this->keyList.size();
if(index>=13){
    index = this->keyList.size() - 5;
    if (this->keyList[index] == this->zobristKey) return true;

    index = this->keyList.size() - 9;
    if (this->keyList[index] == this->zobristKey) return true;

    index = this->keyList.size() - 13;
...
// 新增：將目前棋盤輸出成 FEN 字串（僅棋子擺放部分）
// 由上而下 y=0..9，由左而右 x=0..8；棋盤索引 = (y+3)*16 + (x+3)
std::string Position::toFen() {
  static const char *names = "KABNRCP"; // 型別 0..6 對應 將士象馬車炮兵
  std::string fen;
  for (int y = 0; y <= 9; y ++) {
    int empty = 0;
    for (int x = 0; x <= 8; x ++) {
      unsigned char pc = this->squares[coordToSq(x, y)];
      if (pc == 0) {
        empty ++;
        continue;
      }
      ...
      char ch = names[pc & 7];       // 紅黑同型別皆對應同一字母
      if (pc < 16) fen += ch;         // 紅方（<16）大寫
      else fen += (char)(ch - 'A' + 'a'); // 黑方小寫
    }
