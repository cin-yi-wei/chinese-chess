...
*/
  ...
  std::vector< unsigned short> mvs = this->pos->generateMoves();	// 生成当前局面的所有走法
  unsigned short mv = 0;
  short value = 0;
...
*/
  ...
  for (int i = 0; i < mvs.size(); i ++) {
    ...
    if(this->pos->SRC(mv)<140  || this->pos->DST(mv)<140){ // 129+16
        this->pos->undoMakeMove();
        continue;
 }
...
unsigned short Search::searchMainsecond() { //ALPHA-BETA
  ...
  //this->alphaBetaSearch(-pos->MATE_VALUE, pos->MATE_VALUE, MINMAXDEPTH);
    ...
    for (int i = 1; i <= 64; i ++) {
       ...
       //this->pos->distance = 0;
            // 已经花费的时间 qDebug();
       if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t).count() > time_limit) {
         break;
       }
...
unsigned short Search::searchMain() { //MCTS
  this->mvResult = 0; 	// 搜索出的走法
  mcts MCTS(this->pos);
  std::vector<unsigned short> v=this->pos->generateMoves();
